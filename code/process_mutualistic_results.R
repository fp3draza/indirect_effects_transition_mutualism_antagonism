...
# on the mutualistic networks and compute a series of metrics for further analyses. It will
...
# define function that will take the raw output of the mutualistic coevolution model 
# and calculate the relevant metrics discussed in the paper
process_mutualistic_results <- function(anta_sequence, anta_ratio){
  
  # Parameters
  alpha <- 0.2
  n_sim <- 100
  type <- 'mutualistic_network'
  ...
  write.csv(df, file = paste0('~/indirect_effects_transition_mutualism_antagonism/results/mutualistic_results_', anta_sequence, '_', anta_ratio,'.csv'))
  return(0)
}
...
result = foreach(i=1:nrow(parameter_df)) %dopar% 
  process_mutualistic_results(anta_sequence = parameter_df$strategies[i], 
