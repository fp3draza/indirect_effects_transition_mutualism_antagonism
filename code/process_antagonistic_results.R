...
process_antagonistic_results <- function(anta_sequence){
  ...
  for (i in 1:length(net_files)) {
    ...
    for (anta in seq(0,1,0.2)) {
      ...
      for (replica in 1:n_sim) {
        ...
        result_data <- read.csv(paste0('~/mutualistic_antagonistic_indirect_effects/output/antagonistic_to_mutualistic/all_networks_m_07_a_',a_char,
  ...
  write.csv(df, file = paste0('~/mutualistic_antagonistic_indirect_effects/results/antagonistic_results_', anta_sequence, '.csv'))
  return(0)
}
